...
class BestStoriesFragment : Fragment() {
    ...
    private fun fetchStoryIdList() {
        storiesViewModel.getTopStoriesIDList(VALUE_TYPE_BEST_STORY).observe(viewLifecycleOwner, {
            it?.let { storyIdList ->
                when (storyIdList.status) {
                    Status.SUCCESS -> {
                        storyIdList.data?.let { idList ->
                            cachedStoryIdList = idList
                            Log.d(TAG, "$VALUE_TYPE_BEST_STORY List fetch : $idList")
                    ...
                    Status.ERROR -> {
                        Log.d(TAG, "$VALUE_TYPE_BEST_STORY List fetch : "+storyIdList.message)
                    }
                }
            }
        })
    ...
    private fun fetchStoryById(storyIdList: List<Long>, firstIndex: Int, lastIndex: Int) {
        storiesViewModel.getTopStoriesById(storyIdList[firstIndex].toString(), VALUE_TYPE_BEST_STORY)
                .observe(viewLifecycleOwner, { storyResource ->
                    storyResource?.let { story ->
                        when (story.status) {
                            Status.SUCCESS -> {
                                story.data!!.apply {
                                    storyType = VALUE_TYPE_BEST_STORY
                                }
                                cachedStoryList.add(story.data)
                                Log.d(TAG, "$VALUE_TYPE_BEST_STORY fetch : "+story.data.toString())
                            ...
                            Status.ERROR   -> {
                                binding.progressBar.visibility = View.GONE
                                Log.d(TAG, "$VALUE_TYPE_BEST_STORY fetch : "+story.message)
                            }
                            Status.LOADING -> {
    ...
    private fun fetchFromDB() {
        storiesViewModel.fetchAllStoriesFromDB(VALUE_TYPE_BEST_STORY).observe( viewLifecycleOwner, {
            storyResource ->
            storyResource?.let { story ->
                when (story.status) {
                    Status.SUCCESS -> {
                        ...
                        Log.d(TAG, "Fetch $VALUE_TYPE_BEST_STORY from DB:" + story.data.toString())
                    }
                    Status.ERROR   -> {
                        binding.progressBar.visibility = View.GONE
                        Log.d(TAG, "Fetch $VALUE_TYPE_BEST_STORY from DB:" + story.message)
                    }
                    Status.LOADING -> {
                        binding.progressBar.visibility = View.VISIBLE
                    }
    ...
    private fun insertStories(storyList: List<StoryModel>) {
        storiesViewModel.insertAllStories(storyList).observe( viewLifecycleOwner, {
            ...
            insertAction?.let { insert ->
                when (insert.status) {
                    Status.SUCCESS -> {
                        Log.d(TAG, "${VALUE_TYPE_BEST_STORY}: ${insert.data.orEmpty()}")
                    }
                    Status.LOADING -> {
                        Log.d(TAG, "${VALUE_TYPE_BEST_STORY}: Insert Loading}")
                    }
                    Status.ERROR   -> {
                        Log.d(TAG, "${VALUE_TYPE_BEST_STORY}: ${insert.message.orEmpty()}")
                    }
                }
            }
        })
    ...
    companion object {
        ...
        private const val TAG = "BestStoriesFragment"
    }
